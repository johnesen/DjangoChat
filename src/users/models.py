...
class User(AbstractBaseUser, PermissionsMixin, BaseModel):
    username = models.CharField(
        max_length=150, blank=True, null=True, unique=True, verbose_name=_("username")
    )
    email = models.EmailField(
        blank=False, null=False, unique=True, verbose_name=_("E-Mail")
    )
    photo = models.ImageField(
        upload_to=upload_user_photo_to, null=True, blank=True, verbose_name=_("photo")
    )
    password = models.CharField(
        max_length=128, blank=True, null=True, verbose_name=_("password")
    )
    is_superuser = models.BooleanField(
        default=False, blank=True, verbose_name=_("admin-user")
    )
    ...
    def save(self, *args, **kwargs):
        if self.photo and self.photo != self.photo_:
            self.photo = compress_image(
                self.photo, is_medium_thumbnail=True, quality=80
            )
